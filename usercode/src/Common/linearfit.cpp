...
bool polyUseSimple(int degree, int size, double *dataArray, vector<double> *coefficient)
{
    if(degree >  (size - 1))
    {
        return false;
    ...
    double sx[size];
    double w[] = {0};
    for(int i = 0; i < size; i++) sx[i] = i;
    pf.setAttribute(degree , false , 1.0);
    if( pf.setSample(sx, dataArray, size, false, w) && pf.process() )
    {
        for(int j = 0; j < pf.multiterm ; j++)
        {
            coefficient->push_back(pf.getResult(j));
        }
        return true;
    }
    else
