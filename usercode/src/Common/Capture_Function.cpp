...
void DIO_Capture_data(std::string pinList, int *capDspwave) {
	auto CAP_Data = TheInst.Digital().PatEng().GetCaptureData(pinList);

	for (auto siteData : CAP_Data) {
//		cout << "\n\nSite Number" << dec << siteData.first << endl;
		for (auto pinData : siteData.second) {
//			cout << "\n\nPin Name :" << pinData.first << endl;
			int i = 0;
//			cout << "capture data:  \t" << endl;
			for (auto capData : pinData.second) {
				cout << capData.iECResult;
				capDspwave[i++] = capData.iECResult;
				if (i % 100 == 0) {
//					cout << endl;
				}
			}
